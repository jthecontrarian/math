...
def _policy_filename(total_days, threshold):
    """
    Return the path used to store a policy.

    Example:

        temp/policy_days_730_threshold_4_8.pkl
    """
    threshold_string = str(threshold).replace(".", "_")

    return os.path.join(
        _TEMP_DIR,
        f"policy_days_{total_days}_threshold_{threshold_string}.pkl",
    )


def _policy_config(total_days, threshold):
    """
...
def save_policy(policy, total_days, threshold):
    """
...
def load_policy(total_days, threshold):
    """
...
def is_success(count, deficit, threshold):
    """
    Determine whether the current rating is at or above
    the threshold.

    Rating is represented as:
...
def can_reach_success(
    ...
    To find the absolute best possible outcome, assume that
    every remaining rating is a 5-star rating.

    A 5-star rating:
        - increases count by 1
        - adds zero deficit

    If even this best-case trajectory cannot reach the
    threshold, the state can never become successful while
    keeping.

    Therefore removing it is mathematically exact.
    """

    # A state with no ratings cannot currently be successful.
    #
    # It can only become successful if at least one future
    # rating can arrive.
    if count == 0:
    ...
    # every remaining day produces a 5-star rating.
    best_count = count + remaining_days
...
def action_value(
    ...
    Therefore this function evaluates one complete day.
...
def build_optimal_policy(
    ...
    #     deficit = 0
    #
    # We begin there and expand the reachable state space.
    # --------------------------------------------------------
    ...
    for day in range(total_days, 0, -1):
        ...
        # IMPORTANT:
        #
        # Pruning is exact. A state is retained if it can still
        # possibly achieve the threshold assuming all future
        # ratings are 5 stars.
        # ----------------------------------------------------
...
def get_optimal_action(
    ...
    if key not in _POLICY_CACHE:
        ...
        if policy is None:
            ...
            print(
                "No valid saved policy found."
            )

            print(
                "Calculating optimal MDP policy..."
            )
